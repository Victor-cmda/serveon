...
interface UnitMeasureFormProps {
  ...
  onSuccess?: (unitMeasure: any) => void;
  onCancel?: () => void;
}
...
const UnitMeasureForm = ({
  ...
  onSuccess,
  onCancel,
}: UnitMeasureFormProps) => {
  const navigate = useNavigate();
  const { id: paramId } = useParams();
  const id = mode === 'dialog' ? unitMeasureId : paramId ? Number(paramId) : undefined;
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      let result;
      if (id) {
        result = await unitMeasureApi.update(Number(id), unitMeasureData);
        toast.success('Unidade de medida atualizada com sucesso!');
      } else {
        result = await unitMeasureApi.create(unitMeasureData);
