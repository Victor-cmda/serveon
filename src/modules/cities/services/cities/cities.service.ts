...
export class CitiesService {
    ...
    async create(createCityDto: CreateCityDto): Promise<City> {
        try {
            ...
            if (estadoExiste.rowCount === 0) {
                throw new NotFoundException(`Estado com ID ${createCityDto.estadoId} não encontrado`);
            }
            ...
            if (createCityDto.codigoIbge) {
                ...
                if (cidadeExistente.rowCount > 0) {
                    throw new ConflictException(`Cidade com código IBGE ${createCityDto.codigoIbge} já existe`);
                }
            }
            ...
            if (cidadeComMesmoNome.rowCount > 0) {
                throw new ConflictException(`Cidade com nome ${createCityDto.nome} já existe neste estado`);
            }
    ...
    async findByIbgeCode(codigoIbge: string): Promise<City> {
      ...
      `, [codigoIbge]);
            ...
            if (resultado.rowCount === 0) {
                throw new NotFoundException(`Cidade com código IBGE ${codigoIbge} não encontrada`);
            }
            ...
            console.error(`Erro ao buscar cidade com código IBGE ${codigoIbge}:`, error);
            throw new InternalServerErrorException(`Erro ao buscar cidade com código IBGE ${codigoIbge}`);
        }
    }
    ...
    async update(id: number, updateCityDto: UpdateCityDto): Promise<City> {
        try {
            ...
            if (updateCityDto.estadoId) {
                ...
                if (estadoExiste.rowCount === 0) {
                    throw new NotFoundException(`Estado com ID ${updateCityDto.estadoId} não encontrado`);
                }
                ...
                if (updateCityDto.nome) {
                    ...
                    if (cidadeComMesmoNome.rowCount > 0) {
                        throw new ConflictException(`Cidade com nome ${updateCityDto.nome} já existe neste estado`);
                    }
                }
            ...
            if (updateCityDto.codigoIbge) {
                ...
                if (cidadeExistente.rowCount > 0) {
                    throw new ConflictException(`Cidade com código IBGE ${updateCityDto.codigoIbge} já existe`);
                }
            }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            if (emitentes.rowCount > 0) {
                throw new ConflictException(`Não é possível excluir a cidade, pois ela está vinculada a emitentes`);
            }
            ...
            if (destinatarios.rowCount > 0) {
                throw new ConflictException(`Não é possível excluir a cidade, pois ela está vinculada a destinatários`);
            }
            ...
            if (transportadores.rowCount > 0) {
                throw new ConflictException(`Não é possível excluir a cidade, pois ela está vinculada a transportadores`);
            }

            // Excluir a cidade
            await this.databaseService.query(
                'DELETE FROM dbo.cidade WHERE id = $1',
                [id]
            );
        } catch (error) {
            if (error instanceof NotFoundException || error instanceof ConflictException) {
