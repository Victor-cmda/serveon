...
export class CountriesService {
    ...
    async findByCode(code: string): Promise<Country> {
        try {
            ...
            throw new InternalServerErrorException(`Erro ao buscar país com código ${code}`);
        }
    }
    ...
    async findBySigla(sigla: string): Promise<Country> {
        try {
            ...
            throw new InternalServerErrorException(`Erro ao buscar país com sigla ${sigla}`);
        }
    }
        ...
        try {
            ...
            if (updateCountryDto.codigo) {
                ...
                if (existingCode.rowCount > 0) {
                    throw new ConflictException(`País com código ${updateCountryDto.codigo} já existe`);
                }
            }
            ...
            if (updateCountryDto.sigla) {
                ...
                if (existingSigla.rowCount > 0) {
                    throw new ConflictException(`País com sigla ${updateCountryDto.sigla} já existe`);
                }
            }
