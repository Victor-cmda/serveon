...
export class StatesService {
    ...
    try {
      ...
      if (existingCountry.rowCount === 0) {
        throw new NotFoundException(
          `País com ID ${createStateDto.paisId} não encontrado`,
        );
      }
      ...
      if (existingState.rowCount > 0) {
        throw new ConflictException(
          `Estado com UF ${createStateDto.uf} já existe para o país informado`,
        );
      }
  ...
  async findAllByCountry(paisId: number): Promise<State[]> {
    try {
      ...
      throw new InternalServerErrorException(
        `Erro ao buscar estados do país ${paisId}`,
      );
    }
  }
  ...
  async findByUf(uf: string, paisId: number): Promise<State> {
    try {
      ...
      if (result.rowCount === 0) {
        throw new NotFoundException(
          `Estado com UF ${uf} não encontrado para o país informado`,
        );
      }
      ...
      throw new InternalServerErrorException(
        `Erro ao buscar estado com UF ${uf}`,
      );
    }
  }
    ...
    try {
      ...
      if (updateStateDto.uf && updateStateDto.paisId) {
        ...
        if (existingUf.rowCount > 0) {
          throw new ConflictException(
            `Estado com UF ${updateStateDto.uf} já existe para o país informado`,
          );
        }
        ...
        if (existingUf.rowCount > 0) {
          throw new ConflictException(
            `Estado com UF ${uf} já existe para o país informado`,
          );
        }
      }
      ...
      if (updateStateDto.paisId) {
        ...
        if (existingCountry.rowCount === 0) {
          throw new NotFoundException(
            `País com ID ${updateStateDto.paisId} não encontrado`,
          );
        }
      }
  ...
  async remove(id: number): Promise<void> {
    try {
      ...
      await this.databaseService.query('DELETE FROM dbo.estado WHERE id = $1', [
        id,
      ]);
    } catch (error) {
      if (
