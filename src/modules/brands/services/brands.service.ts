    ...
    try {
      ...
      try {
        ...
        if (existingBrand.rowCount > 0) {
          throw new ConflictException(`Marca '${createBrandDto.nome}' já existe`);
        }
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao criar marca');
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  async findAll(): Promise<Brand[]> {
    try {
      ...
      try {
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao buscar marcas');
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  async findOne(id: number): Promise<Brand> {
    try {
      ...
      try {
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao buscar marca');
      } finally {
        client.release();
      }
    } catch (error) {
      ...
      try {
        ...
        if (updateBrandDto.nome) {
          ...
          if (nameConflict.rowCount > 0) {
            throw new ConflictException(`Nome '${updateBrandDto.nome}' já está sendo usado por outra marca`);
          }
        }
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao atualizar marca');
      } finally {
        client.release();
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (productsUsingBrand.rowCount > 0) {
          throw new ConflictException('Não é possível remover marca que está sendo usada por produtos ativos');
        }

        // Fazer soft delete (marcar como inativo)
        await client.query(
          'UPDATE dbo.marca SET ativo = false WHERE id = $1',
          [id]
        );
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao remover marca');
      } finally {
        client.release();
      }
    } catch (error) {
