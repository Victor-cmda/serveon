...
export class UnitMeasuresService {
    ...
    try {
      ...
      try {
        ...
        if (existingUnitMeasure.rowCount > 0) {
          throw new ConflictException(`Unidade de medida '${createUnitMeasureDto.sigla}' já existe`);
        }
        ...
        const result = await client.query(
          ...
          [
            ...
            createUnitMeasureDto.ativo !== undefined ? createUnitMeasureDto.ativo : true
          ]
        );
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao criar unidade de medida');
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  async findAll(): Promise<UnitMeasure[]> {
    try {
      ...
      try {
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao buscar unidades de medida');
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  async findOne(id: number): Promise<UnitMeasure> {
    try {
      ...
      try {
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao buscar unidade de medida');
      } finally {
        client.release();
      }
    } catch (error) {
  ...
  async update(id: number, updateUnitMeasureDto: UpdateUnitMeasureDto): Promise<UnitMeasure> {
    try {
      const client = await this.databaseService.getClient();
      ...
      try {
        ...
        if (updateUnitMeasureDto.sigla) {
          ...
          if (siglaConflict.rowCount > 0) {
            throw new ConflictException(`Sigla '${updateUnitMeasureDto.sigla}' já está sendo usada por outra unidade de medida`);
          }
        }
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao atualizar unidade de medida');
      } finally {
        client.release();
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (productsUsingUnitMeasure.rowCount > 0) {
          throw new ConflictException('Não é possível remover unidade de medida que está sendo usada por produtos ativos');
        }
        ...
        throw new InternalServerErrorException('Erro interno do servidor ao remover unidade de medida');
      } finally {
        client.release();
      }
    } catch (error) {
